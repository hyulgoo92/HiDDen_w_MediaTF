...
def train(model: Hidden,
    ...
    for epoch in range(train_options.start_epoch, train_options.number_of_epochs + 1):
        ...
        decoder = model.encoder_decoder.decoder  # 혹은 model.decoder

        # 평가 모드로 전환
        decoder.eval()

        # 예시 입력 하나 준비 (noised 이미지의 shape에 맞게)
        example_input = torch.randn(1, 3, 128, 128).to(device)  # 너가 사용하는 이미지 크기와 채널 수에 맞춰줘

        # TorchScript 변환 (script 또는 trace)
        traced_decoder = torch.jit.script(decoder, example_input)

        # 저장
        traced_decoder.save("decoder_only.pth")
